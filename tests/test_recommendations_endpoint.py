...
from rssa_api.data.schemas.movie_schemas import MovieDetailSchema, EmotionsSchema
...
async def test_recommendations_endpoint_returns_emotions(client, db_session):
    ...
    emotions = EmotionsSchema(
        ...
        movielens_id='101',
        anger=0.1, anticipation=0.2, disgust=0.1, fear=0.1, 
        joy=0.5, surprise=0.1, sadness=0.1, trust=0.1,
        iers_count=1, iers_rank=1
    )
    ...
    movie_detail = MovieDetailSchema(
        ...
        tmdb_avg_rating=5.0, tmdb_rate_count=100,
        movielens_avg_rating=5.0, movielens_rate_count=100,
        origin_country="US", parental_guide="PG",
        movie_lens_dataset="25m",
        imdb_genres="Comedy", tmdb_genres="Comedy",
        imdb_popularity=1.0, tmdb_popularity=1.0
    )
    
    response_obj = EnrichedRecResponse(
        recommendations=[movie_detail],
        total_count=1
    )
