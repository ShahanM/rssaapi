...
class AlternateRS(RSSABase):
	def __init__(self, model_path: str, item_popularity, ave_item_score):
		...
		self.discounting_factor = self.__init_discounting_factor(item_popularity)
	...
	def get_predictions(self, ratings: List[RatedItemSchema], user_id: str) -> pd.DataFrame:
		"""
		Parameters
		----------
		ratings: List of RatedItemSchema
		...
		_preds = predict_discounted(self.model, self.item_popularity, user_id, new_ratings, self.discounting_factor)

		return _preds[~_preds['item'].isin(rated_items)]

	def predict_user_topN(self, ratings: List[RatedItemSchema], user_id, n=10) -> List[int]:
		topN_discounted = self.get_predictions(ratings, user_id).head(n)

		return list(map(int, topN_discounted['item']))

	def predict_user_hate_items(self, ratings: List[RatedItemSchema], user_id, n=10) -> List[int]:
		preds = self.get_predictions(ratings, user_id)

		preds = pd.merge(preds, self.ave_item_score, how='left', on='item')
		preds['margin_discounted'] = preds['ave_discounted_score'] - preds['discounted_score']

		preds = preds.sort_values(by='margin_discounted', ascending=False).head(n)

		return list(map(int, preds['item']))

	def predict_user_hip_items(self, ratings: List[RatedItemSchema], user_id, n=10) -> List[str]:
		num_bs = 1000
	...
	def predict_user_no_clue_items(self, ratings: List[RatedItemSchema], user_id, n=10) -> List[str]:
		...
		resampled = resampled.sort_values(by='std', ascending=False).head(n)
	...
	def predict_user_controversial_items(self, ratings: List[RatedItemSchema], user_id, numRec=10) -> List[str]:
		...
		neighbors = self._find_neighbors(umat, users, user_features, distance_method, numNeighbors)
	...
	def __high_std(self, liveUserID, new_ratings):
		...
		for i in range(1, n_models + 1):
			...
			resampled_preds = model.predict_for_user(liveUserID, items_in_sample, new_ratings)
	...
	def __controversial(self, users_neighbor):
		...
		for neighbor in users_neighbor:
			scores_df[str(neighbor)] = self.model.predict_for_user(neighbor, self.items).to_numpy()
