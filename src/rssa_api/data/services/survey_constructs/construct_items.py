...
from rssa_api.data.models.survey_constructs import SurveyItem
from rssa_api.data.repositories.survey_constructs import SurveyItemRepository
...
class SurveyItemService:
    def __init__(
        self,
        item_repo: SurveyItemRepository,
    ):
        self.repo = item_repo
    ...
    async def get_item_by_construct_id(self, construct_id: uuid.UUID) -> list[OrderedTextListItem]:
        items = await self.repo.get_all_by_field('survey_construct_id', construct_id)
    ...
    async def reorder_items(self, construct_id: uuid.UUID, items_map: dict) -> None:
        ...
        Returns:
            None
        """
        await self.repo.reorder_ordered_instances(construct_id, items_map)
