...
class MovieService:
    ...
    async def get_movies_with_emotions_from_ids(self, movie_ids: list[uuid.UUID]) -> list[Movie]:
        movies = await self.movie_repo.get_all_from_ids(movie_ids, options=MovieRepository.LOAD_EMOTIONS)
        if not movies:
            return []
        return movies
    ...
    async def get_movie_details_by_movielens_id(self, movielens_id: str) -> MovieDetailSchema:
        movie = await self.movie_repo.get_by_field(
            'movielens_id', movielens_id, options=MovieRepository.LOAD_FULL_DETAILS
        )
    ...
    async def get_movies(self, limit: int, offset: int, ordering: str = 'none') -> list[MovieSchema]:
        movies = await self.movie_repo.get_paged(limit, offset, ordering)
        if not movies:
            return []
        return [MovieSchema.model_validate(movie) for movie in movies]
