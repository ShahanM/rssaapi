...
REGISTRY = {
    # --- Implicit Models ---
    "implicit_recs_top_n": LambdaStrategy(
        function_name=LAMBDA_IMPLICIT,
        payload_template={'path': 'top_n'}
    ),
    "implicit_recs_discounted_top_n": LambdaStrategy(
        function_name=LAMBDA_IMPLICIT,
        payload_template={'path': 'discounted_top_n'}
    ),
    ...
    "community_advisors": LambdaStrategy(function_name=LAMBDA_IMPLICIT, payload_template={'path': 'community_advisors'}),


    # --- Biased Models ---
    "biased_recs_top_n": LambdaStrategy(
        function_name=LAMBDA_BIASED,
        payload_template={'path': 'top_n'} 
    ),
    "biased_recs_diverse_community_score": LambdaStrategy(
        function_name=LAMBDA_BIASED,
        payload_template={'path': 'diverse_community_score'}
    ),
    "biased_recs_reference_community_score": LambdaStrategy(
        function_name=LAMBDA_BIASED,
        payload_template={'path': 'reference_community_score'}
    ),
    
    # --- Emotion Models ---
    "implicit_ers_top_n": LambdaStrategy(
        function_name=LAMBDA_EMOTION,
        payload_template={'path': 'top_n'}
    ),
    "implicit_ers_diverse_n": LambdaStrategy(
        function_name=LAMBDA_EMOTION,
        payload_template={'path': 'diverse_n'}
    ),
}
