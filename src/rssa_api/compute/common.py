...
from lenskit.algorithms.als import _train_implicit_row_lu as implicit_v_lu
from lenskit.algorithms.als import _train_bias_row_lu as bias_v_lu
from lenskit.algorithms.mf_common import MFPredictor
from scipy.spatial.distance import cosine


def predict(model: MFPredictor, items: pd.DataFrame, user_id: str, ratings: pd.Series) -> pd.DataFrame:
...
def predict_discounted(
	model: ImplicitMF, items: pd.DataFrame, userid: str, ratings: pd.Series, factor: int, coeff: float = 0.5
) -> pd.DataFrame:
...
def get_user_feature_from_implicitMF(model: ImplicitMF, ratings):
	ri_idxes = model.item_index_.get_indexer_for(ratings.index)
	ri_good = ri_idxes >= 0
	ri_it = ri_idxes[ri_good]
	ri_val = ratings.values[ri_good]
	ri_val *= model.weight
	return implicit_v_lu(ri_it, ri_val, model.item_features_, model.OtOr_)


def get_user_feature_from_biasedMF(model: BiasedMF, ratings):
	ri_idxes = model.item_index_.get_indexer_for(ratings.index)
	ri_good = ri_idxes >= 0
	ri_it = ri_idxes[ri_good]
	ri_val = ratings.values[ri_good]
	ureg = model.regularization
	return bias_v_lu(ri_it, ri_val, model.item_features_, ureg)
