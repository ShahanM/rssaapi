...
api_key_study_id_header = APIKeyHeader(
    name='X-Study-Id',
    scheme_name='Study ID',
    description='The ID of the study to retrieve (must be a valid UUID, e.g., a0a0a0a0-a0a0-a0a0-a0a0-a0a0a0a0a0a0)',
)


async def get_current_registered_study(
    study_id_header_value: Annotated[str, Depends(api_key_study_id_header)],
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=f'Invalid Study ID format: "{study_id_header_value}" is not a valid UUID.',
        ) from val_err
    ...
    if not study:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Study with ID '{study_id}' not found")
