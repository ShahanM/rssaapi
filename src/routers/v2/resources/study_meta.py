...
# @router.post('/study/', response_model=StudySchema, tags=[Tags.meta])
# async def new_study(new_study: CreateStudySchema, db: Session = Depends(rssa_db), current_user=Depends(auth0_user)):
...
# @router.post('/study/{study_id}', response_model=StudySchema, tags=[Tags.meta])
# async def dupe_study(study_id: str, db: Session = Depends(rssa_db), current_user=Depends(auth0_user)):
...
# @router.get('/studycondition/{study_id}', response_model=List[StudyConditionSchema], tags=[Tags.meta])
# async def retrieve_conditions(study_id: str, db: Session = Depends(rssa_db), current_user=Depends(auth0_user)):
...
# @router.post('/studycondition/', response_model=StudyConditionSchema, tags=[Tags.meta])
# async def new_condition(
# 	new_condition: CreateStudyConditionSchema, db: Session = Depends(rssa_db), current_user=Depends(auth0_user)
# ):
...
# @router.get('/page/{step_id}', response_model=List[StepPageSchema], tags=[Tags.study])
# async def retrieve_pages(step_id: str, db: Session = Depends(rssa_db), current_user=Depends(auth0_user)):
...
# @router.post('/page/', response_model=StepPageSchema, tags=[Tags.meta])
# async def new_page(new_page: CreatePageSchema, db: Session = Depends(rssa_db), current_user=Depends(auth0_user)):
...
# @router.get('/pagecontent/{page_id}', response_model=List[SurveyConstructSchema], tags=[Tags.meta])
# async def retrieve_page_content(page_id: str, db: Session = Depends(rssa_db), current_user=Depends(auth0_user)):
...
# async def attach_content_to_page(
# 	page_content: CreatePageContentSchema, db: Session = Depends(rssa_db), current_user=Depends(auth0_user)
# ):
...
# 		id=construct.id, name=construct.name, desc=construct.desc, type=construct_type, scale=construct.scale
# 	)
...
# @router.get('/construct/', response_model=List[SurveyConstructSchema], tags=[Tags.meta])
# async def retrieve_constructs(db: Session = Depends(rssa_db), current_user=Depends(auth0_user)):
...
# @router.post('/construct/', response_model=SurveyConstructSchema, tags=[Tags.meta])
# async def new_construct(
# 	new_construct: NewSurveyConstructSchema, db: Session = Depends(rssa_db), current_user=Depends(auth0_user)
# ):
# 	if new_construct.scale_id == '':
# 		construct = create_text_construct(db, new_construct.name, new_construct.desc, new_construct.type_id)
# 	else:
# 		construct = create_survey_construct(
# 			db, new_construct.name, new_construct.desc, new_construct.type_id, uuid.UUID(new_construct.scale_id)
# 		)
...
# @router.put('/construct/{construct_id}', response_model=SurveyConstructSchema, tags=[Tags.meta])
# async def update_construct(
# 	construct_id: str,
# 	updated_construct: UpdateSurveyConstructSchema,
...
# @router.get('/construct/{construct_id}', response_model=SurveyConstructDetailSchema, tags=[Tags.meta])
# async def retrieve_construct_details(
# 	construct_id: str, db: Session = Depends(rssa_db), current_user=Depends(auth0_user)
# ):
...
# 			scale_levels=[
# 				ScaleLevelSchema(level=level.level, label=level.label, scale_id=level.scale_id)
...
# 		name=construct.name,
# 		desc=construct.desc,
# 		type=ConstructTypeSchema.model_validate(construct_type) if construct_type else None,
...
# @router.get('/constructtype/', response_model=List[ConstructItemTypeSchema], tags=[Tags.meta])
# async def retrieve_construct_types(db: Session = Depends(rssa_db), current_user=Depends(auth0_user)):
...
# async def new_construct_type(
# 	new_type: NewConstructTypeSchema, db: Session = Depends(rssa_db), current_user=Depends(auth0_user)
# ):
...
# async def retrieve_construct_scales(db: Session = Depends(rssa_db), current_user=Depends(auth0_user)):
...
# async def new_construct_scale(
# 	new_scale: NewConstructScaleSchema, db: Session = Depends(rssa_db), current_user=Depends(auth0_user)
# ):
...
# async def retrieve_construct_items(construct_id: str, db: Session = Depends(rssa_db), current_user=Depends(auth0_user)):
...
# async def new_construct_item(
# 	new_item: CreateConstructItemSchema, db: Session = Depends(rssa_db), current_user=Depends(auth0_user)
# ):
...
# async def retrieve_construct_item_types(db: Session = Depends(rssa_db), current_user=Depends(auth0_user)):
...
# async def new_construct_item_type(
# 	new_type: NewConstructItemTypeSchema, db: Session = Depends(rssa_db), current_user=Depends(auth0_user)
# ):
