...
from data.models.schemas.movieschema import MovieSchemaV2, EmotionsSchema
...
def get_ers_movies_by_movielens_ids(db: Session, movielens_ids: list[str]) -> list[MovieSchemaV2]:
	...
	movie_dict = {movie.movielens_id: {'movie': movie, 'emotions': emotions} for movie, emotions in results}
...
def get_movie_recommendation_text(db: Session, movie_id: uuid.UUID) -> Union[MovieRecommendationSchema, None]:
	db_response = db.query(MovieRecommendationText).filter(MovieRecommendationText.movie_id == movie_id).first()
