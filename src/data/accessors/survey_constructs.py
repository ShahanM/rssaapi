...
def create_construct_scale(db: Session, levels: int, name: str, scale_levels: List[NewScaleLevelSchema]) -> ConstructScale:
	...
	for scale_level in scale_levels:
		new_scale_level = ScaleLevel(level=scale_level.level, label=scale_level.label,
				scale_id=scale.id)
...
def create_construct_item(db: Session, construct_id: uuid.UUID, item_type: uuid.UUID, text: str,
		order_position: int) -> ConstructItem:
	construct = get_survey_construct_by_id(db, construct_id)
	itype = get_item_type_by_id(db, item_type)
	item = ConstructItem(construct_id=construct.id, text=text,
			order_position=order_position, item_type=itype.id)
	db.add(item)
	db.commit()
	db.refresh(item)
