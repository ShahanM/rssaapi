...
def load_training_data(data_path):
	...
	ratings_train = ratings_train\
		.rename(columns = {'user_id': 'user', 'movie_id': 'item'})
...
def load_training_data_npz(data_path):
	...
	trainset = trainset.astype(\
		{'user': int, 'item': int, 'rating': float, 'timestamp': int})
...
def __train_resampled_models(training_data: pd.DataFrame, algo: str, \
	resample_count: int, output_dir: str, alpha: float = 0.5):
...
def __compute_ave_item_scores(model, training_data, item_popularity, alpha=0.2):
	...
	for user in users:
		...
		user_df['discounted_score'] = user_df['score'] \
			- alpha*(user_df['count']/discounting_factor)
				
		ave_scores_df['ave_score'] = (ave_scores_df['ave_score'] \
				* calculated_users + user_df['score'])/(calculated_users + 1)
		
		ave_scores_df['ave_discounted_score'] = \
			(ave_scores_df['ave_discounted_score'] * calculated_users \
			+ user_df['discounted_score'])/(calculated_users + 1)
...
def __main(data_path: str, model_path: str, algo: str, \
	item_popularity: bool, ave_item_score: bool, resample_count: int):
	...
	item_ratings = sorted(zip(items, rating_counts), key = lambda x: x[1], \
		reverse = True)
	...
	items_popularity = pd.DataFrame(itemranks, \
				columns = ['item', 'count', 'rank'])
	
	# 1.2 - Start to discounting the input ratings by ranking
	b = 0.4
	rpopularity = pd.merge(train_data, items_popularity, how='left', on='item')
	rpopularity['discounted_rating'] = rpopularity['rating'] \
		* (1-b/(2*rpopularity['rank']))
	...
	if item_popularity:
		...
		items_popularity.to_csv(f'{model_path}/item_popularity.csv', \
			index = False)
...
if __name__ == '__main__':
	parser = argparse.ArgumentParser(\
		description='Train MF models for RSSA algorithms.')
	...
	parser.add_argument('-o', '--model_path', type=str, required=True, \
		help='Path to the folder to save the trained model.')
	parser.add_argument('-a', '--algo', type=str, required=True, \
		choices=['implicit', 'explicit'], \
		help='Specify the MF algorithm to use.')
	
	parser.add_argument('--item_popularity', type=bool, required=True, \
		action=argparse.BooleanOptionalAction, default=False, \
		help='Specify whether to save the item popularity as a csv file.')
	parser.add_argument('--ave_item_score', type=bool, required=True, \
		action=argparse.BooleanOptionalAction, default=False, \
		help='Specify whether to generate the average item score for users.')
	
	parser.add_argument('-r', '--resample_count', type=int, required=False, \
	...
	__main(args.data_path, args.model_path, args.algo, args.item_popularity, \
		args.ave_item_score, args.resample_count)
