"""
----
File: pref_viz_service.py
Project: RS:SA Recommender System (Clemson University)
Created Date: Sunday, 12th October 2025
Author: Mehtab 'Shahan' Iqbal
Affiliation: Clemson University
----
Last Modified: Tuesday, 14th October 2025 6:36:15 pm
...
class PreferenceVisualization(RSSABase):
    ...
    def get_prediction(
        ...
        preds_df = self._predict(user_id, ratings)
    ...
    def predict_diverse_items(
        ...
        user_id: str,
        algo: str = 'fishnet',
        # Eliminate redundant flags; assume randomization is handled by seed or is implicit
    ...
    def get_candidates(
        ...
        if ave_score_type == 'nn_predicted':
            ...
            user_features = self.get_user_feature_vector(ratings)
            search_space_k = 200
            nn_ids: list[int] = self.annoy_index.get_nns_by_vector(
                user_features, search_space_k, include_distances=False
            )
        ...
        if not baseline_df.empty:
            candidates = pd.merge(
                candidates, baseline_df[['item', 'ave_score']], how='left', on='item', suffixes=('_user', '_baseline')
            )
    ...
    def calculate_predicted_neighborhood_average(
        ...
        """
        Computes the Predicted Neighborhood Average (P_NN_Ave) for a list of target items.

        Args:
            neighbor_internal_codes (np.ndarray): The internal indices (Annoy IDs) of the K neighbors.
            target_item_uuids (List[str]): The external UUIDs of the N items to be averaged.

        Returns:
            pd.DataFrame: DataFrame containing ['item', 'p_nn_ave_score'] ready for merging.
        """

        # Get the Sliced Q Matrix (Target Items)
        Q_target_slice, valid_item_ids = self._get_target_item_factors(target_item_ids)

        # Get the Sliced P Matrix (K Neighbors)
        P_full_tensor = self.scorer.user_features_
        if P_full_tensor is None:
            raise RuntimeError('Something went wrong')

        P_full_numpy = P_full_tensor.cpu().detach().numpy()
        P_neighbors_slice = P_full_numpy[neighbor_internal_codes, :]  # K x F

        Prediction_Matrix = P_neighbors_slice @ Q_target_slice.T

        P_NN_Ave_Vector = np.mean(Prediction_Matrix, axis=0)  # Shape: (N,)

        p_nn_ave_df = pd.DataFrame(
            {'p_nn_ave_score': P_NN_Ave_Vector},
            index=valid_item_ids,
        )
