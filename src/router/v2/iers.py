...
async def create_recommendations(rated_movies: NewRatingSchema, db: Session = Depends(movie_db_v2)):
...
async def update_recommendations(rated_movies: EmotionInputSchema, db: Session = Depends(movie_db_v2)):
	iers_item_pop, iersg20 = get_iers_data()
	iers_model_path = get_iers_model_path()
	iersalgs = EmotionsRS(iers_model_path, iers_item_pop, iersg20)
	recs = []
...
async def update_recommendations_experimental(
	rated_movies: EmotionInputSchemaExperimental, db: Session = Depends(movie_db_v2)
):
	iers_item_pop, iersg20 = get_iers_data()
	iers_model_path = get_iers_model_path()
	iersalgs = EmotionsRS(iers_model_path, iers_item_pop, iersg20)
