...
async def create_cybered_recommendations(rated_movies: RatingsSchema, db: Session = Depends(get_cybered_db)):
    ...
    rcs = cybered.get_condition_prediction(rated_movies.ratings, rated_movies.user_id, rated_movies.rec_type, 144)
    rcsm = get_movies_by_ids(db, rcs)
    pd.DataFrame(rcsm).to_csv('recs.csv')

    recs = cybered.get_condition_prediction(
        rated_movies.ratings, rated_movies.user_id, rated_movies.rec_type, rated_movies.num_rec
    )
